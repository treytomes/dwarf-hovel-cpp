...
void MapTestState::render(IRenderContext* context, unsigned int delta_time_ms) {
	...
	// Draw the grid underneath everything.
	...
	// Draw particles on top of the entities.

	if (fountain != nullptr) {
		fountain->draw(context, delta_time_ms);
	} else {
		bitmaps::bush.draw(context, PointUI(50, 150));
	}

	// Make sure the UI gets rendered on top of *everything*.
